...
**一言サマリ**:
- 両方入れておけば **Claude が自動で選ぶ**。基本はこれでOK
- Chat / Sheets / Docs など **Connector 未対応のサービスは gws-skill 一択** なので、その時だけ `/gws-*` と明示

---

## Connectors で扱えるサービス (2026年4月時点)

公式の Anthropic Connectors で提供されているツール群:

| サービス | 主な操作 |
|---|---|
| Gmail | `search_threads` / `get_thread` / `create_draft` / `list_drafts` / `create_label` / `list_labels` |
| Calendar | `list_events` / `create_event` / `update_event` / `delete_event` / `respond_to_event` / `suggest_time` |
| Drive | `search_files` / `read_file_content` / `create_file` / `download_file_content` / `get_file_metadata` / `list_recent_files` |

**未対応**(= gws-skill が必要):
- Google Sheets (読み書き・行追加)
- Google Docs (読み書き)
- Google Chat (メッセージ送信)
- Google Meet (会議管理)
- Google Tasks / Slides / Forms / Keep / Classroom など
